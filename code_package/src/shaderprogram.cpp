...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      attrJointid(-1),attrWeights(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1),
      unifTime(-1),unifRenderMode(-1),unifBindMatrix(-1),unifOverallTransformation(-1),
      context(context)
{}
...
void ShaderProgram::SetTimer(int timeCount)
{
    useMe();
    if(unifTime!= -1) {
    // Pass a 4x4 matrix into a uniform variable in our shader
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrNor != -1 && d.bindNor()) {
        ...
        context->glVertexAttribPointer(attrNor, 3, GL_FLOAT, false, 0, NULL);
    }
    ...
    if(attrWeights!=-1&& d.bindVertex_Weight())
    {
        context->glEnableVertexAttribArray(attrWeights);
        context->glVertexAttribPointer(attrWeights,2,GL_FLOAT,false,0,NULL);
    }
    if(attrJointid!=-1&& d.bindInfluence_ID())
    {
        context->glEnableVertexAttribArray(attrJointid);
//        context->glVertexAttribPointer(attrJointid,2,GL_INT,false,0,NULL);
        context->glVertexAttribPointer(attrJointid,2,GL_FLOAT,false,0,NULL);
    }
    ...
    d.bindIdx();
    if(d.drawMode()==GL_POINTS)
        glPointSize(8.0f);
    if(d.drawMode()==GL_LINES)
        glLineWidth(1.0f);//We cannot change the linewidth in thie opengl context.
    ...
    if(attrWeights!=-1)
        context->glDisableVertexAttribArray(attrWeights);
    if(attrJointid!=-1)
        context->glDisableVertexAttribArray(attrJointid);
